...
var VIS_BASE = "https://storage.googleapis.com/four_m_site/images/uio_comparison";
var VIS_NUM_IMGS = 3;
var vis_img_idx = 0;
var TASK_TITLES = [
...
function setSpecificVisImage(img_idx) {
  var path = VIS_BASE + '/' + String(img_idx) + '.svg';
  document.getElementById('visFrame').src = path
  document.getElementById('task-title').innerHTML = TASK_TITLES[img_idx]
}
...
function prevVisImage() {
  vis_img_idx = mod(vis_img_idx - 1, VIS_NUM_IMGS);
  setSpecificVisImage(vis_img_idx);
}

function nextVisImage() {
  vis_img_idx = (vis_img_idx + 1) % VIS_NUM_IMGS;
  setSpecificVisImage(vis_img_idx);
}
...
$(document).ready(function() {
    ...
    const dataAblationRGB2X = {
      labels: [
        'RGB → RGB', 'RGB → CLIP', 'RGB → Semantic', 'RGB → Geometric', 'RGB → All'
      ],
      datasets: [{
        backgroundColor: [
    ...
    const dataAblationAll2X = {
      labels: [
        'All → RGB', 'All → CLIP', 'All → Semantic', 'All → Geometric', 'All → All'
      ],
      datasets: [{
        backgroundColor: [
          'rgba(181, 228, 140, 0.2)',
  ...
  document.getElementById('matrixFrame').addEventListener('load', function() {
    ...
    if (iframeContent) {
      ...
      texts.forEach(function(text) {
        var textNodes = Array.from(text.childNodes).filter(node => (node.nodeName == 'g' && pattern.test(node.id)));
        textNodes.forEach(function(node) {

          var text_val = '';
          node.childNodes.forEach(function(t) {
            text_val += t.getAttribute('aria-label');
          });

          var index = SRC_CAPTIONS.indexOf(text_val);

          if (index != -1){
            text_val = DST_CAPTIONS[index];
            ...
            node.parentNode.addEventListener('click', function() {
              ...
              // Create and show the popup
              var popup = document.getElementById('popup');
              popup.innerHTML = '<div class="content has-text-centered"><p></p>';

              if (text_val.includes('<br>')){
                popup.childNodes[0].childNodes[0].innerHTML = text_val;
              } else {
                popup.childNodes[0].childNodes[0].textContent = text_val;
              }
              popup.style.display = 'block';
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    clickableRows.forEach(row => {
        row.addEventListener("click", function() {
          ...
          const cell = this.querySelector("td").childNodes[0];
          if (cell.textContent.includes("Show pseudo labeler evaluations")) {
            cell.textContent = "Hide pseudo labeler evaluations";
          } else if (cell.textContent.includes("Hide pseudo labeler evaluations")) {
              cell.textContent = "Show pseudo labeler evaluations";
          }

          var nextRow = this.nextElementSibling;
          while (nextRow.classList.contains("expandable-content") || nextRow.classList.contains("expanded")){
            if (nextRow.classList.contains("expandable-content")) {
              nextRow.classList.remove("expandable-content");
              nextRow.classList.add("expanded");
            } else if (nextRow.classList.contains("expanded")) {
                nextRow.classList.remove("expanded");
                nextRow.classList.add("expandable-content");
            }
